...
def top_share(values: list[float], k: int = 1) -> float:
    if not values:
        return 0.0
    total = sum(values)
    if total <= 0:
        return 0.0
    return round(sum(sorted(values, reverse=True)[:k]) / total, 6)


def strategy_frequency(leaderboard: list[dict[str, Any]]) -> dict[str, int]:
    ctr = Counter(row["strategy"] for row in leaderboard)
    return dict(sorted(ctr.items()))


def _external_action_frequency(action_counts: dict[str, int]) -> dict[str, int]:
    mapped: dict[str, int] = {}
...
def _governance_capture_metrics(result: dict[str, Any]) -> dict[str, Any]:
...
def _timeline_markers(result: dict[str, Any]) -> dict[str, Any]:
...
def _winner_reason(result: dict[str, Any]) -> dict[str, Any]:
    leaderboard = result.get("leaderboard", [])
    if not leaderboard:
...
def compute_metrics(result: dict[str, Any]) -> dict[str, Any]:
    ...
    governance = _governance_capture_metrics(result)
    timeline = _timeline_markers(result)
    winner = _winner_reason(result)
    return {
        "gini_resources": gini(resources),
        "hhi_resources": hhi(resources),
        "top1_share": top_share(resources, 1),
        "top3_share": top_share(resources, 3),
        "alive_count": alive_count,
        "strategy_frequency": strategy_frequency(result["leaderboard"]),
        "action_frequency": _external_action_frequency(result.get("action_counts", {})),
