...
import { AVATAR_API_BASE, STRATEGY_COLORS } from "@/lib/gameConstants";
...
const VisualAgentCard: React.FC<AgentCardProps> = ({ agent }) => {
  ...
  return (
    <div
      className={`bg-slate-800 p-4 rounded-lg border-2 ${agent.isAlive ? STRATEGY_COLORS[agent.strategy] : "border-gray-600"} shadow-lg transition-all`}
    >
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        <div className="bg-slate-900/50 p-2 rounded flex items-center justify-between">
          ...
          <span
            className={`${agent.health < 15 ? "text-red-500" : "text-green-400"} font-mono`}
          >
            {agent.health}/50
          </span>
        </div>
            ...
            <span className="italic">
              {agent.lastAction === "IDLE"
                ...
                : agent.lastAction === "ATTACK"
                  ? `Attacking ${agent.lastActionTarget}!`
                  : agent.lastAction === "STEAL"
                    ? `Stealing from ${agent.lastActionTarget}!`
...
export default VisualAgentCard;
