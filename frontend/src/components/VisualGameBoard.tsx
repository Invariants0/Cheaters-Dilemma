...
import { GameAgent, Position } from "@/lib/gameTypes";
import {
  GRID_SIZE,
  AVATAR_API_BASE,
  STRATEGY_COLORS,
...
interface GameBoardProps {
  agents: GameAgent[];
  onAgentClick: (agent: GameAgent) => void;
  selectedAgentId: string | null;
  showInteractions: boolean;
}

const VisualGameBoard: React.FC<GameBoardProps> = ({
  agents,
  onAgentClick,
  selectedAgentId,
  showInteractions,
  ...
  const getAgentAt = (pos: Position) =>
    agents.find(
      (a) => a.isAlive && a.position.x === pos.x && a.position.y === pos.y,
    );

  const getActionIcon = (agent: GameAgent) => {
    switch (agent.lastAction) {
      case "ATTACK":
        return <Swords size={12} className="text-red-500 animate-pulse" />;
      case "STEAL":
        return <HandCoins size={12} className="text-yellow-500" />;
      case "MOVE":
        return <Footprints size={12} className="text-slate-400" />;
      case "HEAL":
        return <ShieldPlus size={12} className="text-green-500" />;
      default:
        return null;
    }
  };
  ...
  const getInteractionColor = (action: string) => {
    switch (action) {
      case "ATTACK":
        return "text-red-300 bg-red-900/80 border-red-500";
      case "STEAL":
        return "text-yellow-300 bg-yellow-900/80 border-yellow-500";
      case "HEAL":
        return "text-green-300 bg-green-900/80 border-green-500";
      case "GATHER":
        return "text-blue-300 bg-blue-900/80 border-blue-500";
      default:
        return "text-slate-200 bg-slate-900/80 border-slate-500";
    }
  };
          ...
          >
            {agent && (
              <div className="relative w-full h-full p-0.5 group">
                {/* Agent Avatar */}
                <div
                  className={`w-full h-full rounded overflow-hidden border-2 ${STRATEGY_COLORS[agent.strategy]} bg-slate-950`}
                >
                  <img
...
export default VisualGameBoard;
