...
export const generateAgents = (count: number): GameAgent[] => {
...
export const processTick = (
  agents: GameAgent[],
  tickCount: number,
): { newAgents: GameAgent[]; logs: GameLog[] } => {
  let logs: GameLog[] = [];
  ...
  for (const idx of agentIndices) {
    ...
    // AI DECISION MAKING
    // 1. Check neighbors
    const neighbors = nextAgents.filter(
      (n) =>
        n.isAlive &&
        n.id !== agent.id &&
    ...
    if (agent.health < 20) {
      ...
      switch (agent.strategy) {
        ...
        case AgentStrategy.HONEST:
          ...
          if (target.reputation < 30 || target.score > 80)
            action = ActionType.ATTACK;
    ...
    agent.lastActionTarget = targetId
      ? nextAgents.find((a) => a.id === targetId)?.name
      : undefined;
    ...
    switch (action) {
      case ActionType.MOVE: {
        ...
        agent.position.x = Math.max(
          0,
          Math.min(GRID_SIZE - 1, agent.position.x + moveX),
        );
        agent.position.y = Math.max(
          0,
          Math.min(GRID_SIZE - 1, agent.position.y + moveY),
        );
        break;
      }
