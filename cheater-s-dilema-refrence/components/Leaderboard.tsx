...
const Leaderboard: React.FC<LeaderboardProps> = ({ agents }) => {
            ...
            {sortedAgents.map((agent, index) => (
              ...
              >
                <td className="py-2 font-mono text-slate-500 w-6">
                    {index === 0 && agent.score > 0 ? <Crown size={12} className="text-yellow-500 inline"/> : index + 1}
                </td>
                <td className="py-2 font-medium truncate max-w-[100px]">
                    {agent.name}
                    {!agent.isAlive && <Skull size={10} className="inline ml-1 text-red-500"/>}
                </td>
                <td className="py-2 text-right font-mono">
                  <div className="relative">
                     ...
                     <span className="relative z-10 text-[10px]">{Math.floor(agent.score)}</span>
                  </div>
                </td>
              </tr>
            ))}
